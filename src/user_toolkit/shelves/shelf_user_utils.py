...
class load(shelf_base._shelf):
    ...
    def build(self):
        ...
        # Reload shelf button
        self.addButton(label="", icon=ICON_DIR + "/reloadShelf.png", command=lambda: reload_shelf(shelf_name=SHELF_NAME))
        ...
        self.addMenuItem(general_tools_menu, "Explore to Project Directory", command=lambda _: explore_maya_project())

        self.addMenuItemDivider(general_tools_menu, divider=True, dividerLabel='SETUP...')

        self.addMenuItem(general_tools_menu, "Setup Marking Menu", command=lambda _: setup_user_marking_menu())
        ...
        cmds.menuItem(p=week6_tools_menu, l="Duplicate (Parent Only)",
                      command=lambda _: cmds.duplicate(parentOnly=True))

        cmds.menuItem(p=week6_tools_menu, l="Snap second selected object to first",
                      command=lambda _: week6.match_selection())

        cmds.menuItem(p=week6_tools_menu, l="Create Pole Vector (select PV control then IK handle)",
                      command=lambda _: week6.create_pole_vector_from_selection())

        cmds.menuItem(p=week6_tools_menu, l="Create Category Switch (select rig top node)",
                      command=lambda _: week6.create_category_ui())

        cmds.menuItem(p=week6_tools_menu, l="Add offset and group transforms above selected control",
